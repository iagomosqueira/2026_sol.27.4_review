...
### Calculation of ICES reference points

The methodology employed follows that applied in the 2024 benchnmark for the stock (ICES, 2024). The stock-recruiment relationship was classified as type 5 according to ICES guidelines (ICES, 2017), and $B_{lim}$ was computed as the average of the four lowest observed spawning biomass values. The *eqsim* algorithm was again applied to compute reference points, using a bounded segmented regression model, with the inflexion point set to Blim. The last year of recruitment estimates, in this case 2023, has been excluded, as its estimate of considered very uncertain and Stock Synthesis does not fir recuritment deviances in the final year. Future mean weights at age have been taken as an average over the last five years, as a decreasing pattern exists in the sampled stock weights over the last 10 years. Natural mortality and maturity are naturally set at the same level as in all years of the assessment. The exploitation pattern at age was also taken as an average of the last five years, while the future discards ratio was set as an average of the last three years. Code employed for the calculation is attached as Appendix A.
...
Table 2: Analysis of deviance table from a likelihood ratio test comparing the GLM models Landings and LandingsR12.

```
Analysis of Deviance Table

Model 1: discards ~ log(landings)
Model 2: discards ~ log(landings) + log(rec1) + log(rec2)
  Resid. Df Resid. Dev Df Deviance    F Pr(>F)
1        21       2.68
2        19       2.20  2    0.478 2.13   0.15
```

Table 3: Analysis of deviance table from a likelihood ratio test comparing the GLM models Landings, and LandingsR.

```
Analysis of Deviance Table

Model 1: discards ~ log(landings)
Model 2: discards ~ log(landings) + log(rec)
  Resid. Df Resid. Dev Df Deviance    F Pr(>F)
1        21       2.68
2        20       2.07  1    0.609 6.15  0.022
```


## Model predictions

Predictions, or more correctly calculations for the method based on the average ratio, are presented in Figure \@ref(fig:discardspredictions), and then compared with the landings series in Figure \@ref(fig:discardspredictionslandings). All options predict discards that are larger than those observed from 2002, consistent with expectations as total catches were higher. All models using recruitment as a covariate predict very large discard values in the 1978-1982 period, higher than when employing only landings as predictor. Recruitment appears to be linked but only explains a relatively reduced part of the total variance (tables 2 and 3).
...
```{r discardspredictionslandings, fig.cap="Predicted total discards (in weight) from the different models fitted to the observed discards series, and compared to the landings (black line). The dots show the observed discards."}
...
The availability of fish that could be discarded is not only dependent on the number of fish in those ages, but also on their length, as minimum landings size is the main reason for discarding. Changes in mean weight, and length, over the period for which discards are being reconstructed are well known (ICES, 2025). So using only recruitment strength as a predictor of availability could overestimate discards in the period where fish grew larger than the minimum landing size much earlier. This could explain partly the large differences between the GLM with recruitment and AAP in certain years, as AAP included the stock weight-at-age in its calculations.

A discussion on the choice of model, or procedure, took place during the 2026 sesssion of WGNSSK. The agreement was to employ only landings as predictor, given that properlym accounting for availability would require a more complex modelling exercise, while differences in estimates were minor. The GLm linking discards and landings was preferred, as it includes a dynamic element that the simple discard ratio calculation lacks. The proposal is to run this model and assemble a new discard time series that will not be rerun as the dataset grows, unless a completely new reconstruction model is considered, or the process is included in the stock assessment itself.
...
ICES (2025). Working Group on the Assessment of Demersal Stocks in the North Sea and Skagerrak (WGNSSK). ICES Scientific Reports. Report. https://doi.org/10.17895/ices.pub.29085995.v4
...
## Appendix A: Code for the calculation of ICES reference points

```{r, eval=FALSE, echo=TRUE, code=readLines("model_refpts.R")}
```
