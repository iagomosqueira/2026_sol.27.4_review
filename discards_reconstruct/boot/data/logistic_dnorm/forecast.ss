...
#C  generic forecast file (V3.24f) Sole 4
...
0.35 # Biomass target (e.g. 0.40) as fraction of SSB_virgin if depletion basis = 1 or time_vary compatibility = 0, else as fraction of SSB_unfished in benchmark
...
# Bmark_years: beg_bio, end_bio, beg_selex, end_selex, beg_relF, end_relF, beg_recr_dist, end_recr_dist, beg_SRparm, end_SRparm (enter actual year, or values of 0 or -integer to be rel. endyr)
 1970 1980 -999 -999 -9 0 -999 0 -999 0
#  1970 1980 1957 1957 2015 2024 1957 2024 1957 2024
# value <0 convert to endyr-value; except -999 converts to start_yr; must be >=start_yr and <=endyr
1 # Bmark_relF_Basis: 1 = use year range; 2 = set relF same as forecast below
#
3 # Forecast: -1=none; 0=simple_1yr; 1=F(SPR); 2=F(MSY) 3=F(Btgt) or F0.1; 4=Ave F (uses first-last relF yrs); 5=input annual F scalar
...
# 10 1 -2 0 # selectivity; use:  10 1 -2 0
# 11 1 -2 0 # rel_F; use:  11 1 -2 0
# 12 1 -2 0 # recruitment; use:  12 1 -2 0
#-9999 0 0 0
#
...
0.004 # Control rule inflection for constant F (as frac of HCR_anchor, see below); must be > control rule cutoff
0.001 # Control rule cutoff for no F (as frac of HCR_anchor, e.g. 0.10); negative value to also invoke read of protection level 
 # placeholder for AK protection level:  control rule level below which F goes to 0.0001
1 # Buffer:  enter Control rule target as fraction of Flimit (e.g. 0.75), negative value invokes list of [year, scalar]. -year fills from year to YrMax 
# Also see HCR_anchor below to use virgin vs benchmark SSB or Bmsy as basis for inflection and cutoff
#
3 #_N forecast loops (1=OFL only; 2=ABC; 3=get F from forecast ABC catch with allocations applied)
0 # First forecast loop with stochastic recruitment
...
2003 # Rebuilder:  year for current age structure (Yinit) (-1 to set to endyear+1)
...
#
99 # basis for input Fcast catch: -1=read basis with each obs; 2=dead catch; 3=retained catch; 99=input apical_F; NOTE: bio vs num based on fleet's catchunits
